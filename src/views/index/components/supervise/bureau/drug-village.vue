<template>
  <div class="sup-main my-sup-b clearfix">
    ...
    <div class="sup-content-box clearfix">
      ...
      <!--村卫生室药物补助，执行-村卫生室详情-->
      <div class="sup-content" v-if="sideThreeCurP === 9">
        <div class="sup-c3-detail">
          <span>项目年度：{{addDrugNewDetail.year}}</span>
          <span>项目季度：{{addDrugNewDetail.quarter === '9' ? '年终考核' : (addDrugNewDetail.quarter + '季度')}}</span>
          <span>项目代码：{{addDrugNewDetail.quotaNum}}</span>
        ...
        <el-table
          ...
          <el-table-column
            ...
            label="转账回执单">
            <template slot-scope="scope">
              <span v-if="scope.row.appendices" @click="previewPictures(scope.row.appendices[0].fileFullPath, 'bigPic')" style="color: #1890FF">查看图片</span>
              <p v-else>--</p>
            </template>
          </el-table-column>
          <el-table-column
...
export default {
  data () {
    return {
      ...
      breadList: {
        '1': [
          {name: '基本药物制度补助-村卫生室补助', sideType: 1, sideCurP: 1}
        ]
      },
      sideType: 1,
      pagination: { // 分页
      ...
      // 基本药物
      sideThreeCurP: 1,
      drugType: 0, // 1为卫生院药物补助，0为村卫生室药物补助
      drugList: [], // 基础药物补助列表
      drugCurMonth: {
        '00': '1月份',
        '01': '2月份',
  ...
  methods: {
    ...
    drugBudgetDetail (item, addBread) {
      if (addBread) {
        this.drugHospitalExeUid = '';
        this.breadList['1'].push({name: '预算详情', sideType: 1, sideCurP: 5})
    ...
    drugExecuteDetail (item, addBread) {
      if (addBread) {
        ...
        this.breadList['1'].push({name: '执行详情', sideType: 1, sideCurP: 8})
    ...
    drugVillageEntry () {
      ...
      this.breadList['1'].push({name: '选择村卫生室', sideType: 1, sideCurP: 6})
      // 获取该卫计项目选择的卫生院
      let p = {
        where: {
          projectUid: this.addDrugNewDetail.uid,
    ...
    reviseDrugVillageEntry (item) {
      ...
      this.drugCanSelect = true;
      this.sideThreeCurP = 6;
      this.breadList['1'].push({name: '修改村卫生室', sideType: 1, sideCurP: 6})
      // 获取该卫计项目未录入
      let p = {
        where: {
    ...
    drugExecuteVillageDetial (item, addBread) {
      ...
      if (addBread) {
        this.breadList['1'].splice(2, this.breadList['1'].length, {name: '村卫生室详情', sideType: 1, sideCurP: 9})
      }
      // 获取该基础项目选择的村卫生室
      let p = {
        where: {
